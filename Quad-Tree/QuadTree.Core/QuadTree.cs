namespace QuadTree.Core
{
    ...
    public class QuadTree<T> where T : IBoundable
    {
        ...
        private int GetQuadrant(Node<T> node, Rectangle bounds)
        {
            ...
            var horisontalMidPoint = node.Bounds.MidY;

            var inTopQuadrant = node.Bounds.Y1 <= bounds.Y1 && bounds.Y2 <= horisontalMidPoint;
            var inBottomQuadrant = horisontalMidPoint <= bounds.Y1 && bounds.Y2 <= node.Bounds.Y2;
